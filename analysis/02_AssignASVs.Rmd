...
Since I have quite a bit of variation in ASV length, yes I think I think I will allow some variation instead of trimming to an exact length. Based on the ASV length data I've generated above, I think I will trim ASV length to a range of 408bp - 426bp.

## Trim ASV lengths 

Now that we are pretty convinced that we have some spurious ASVs, we will trim ASVs that are higher or lower than 408bp - 426bp base pairs in this data set. 
...
raw_ASV_table_trimmed <- 
  raw_ASV_table[, between(nchar(colnames(raw_ASV_table)), 408, 426)]
...
So, we have removed 7633 ASVs, which were chimeras. Let's review what proportion of ASVs we have retained along the workflow: 
...
- 18.22% of raw ASVs were retained after trimming?
(2) What proportion of the trimmed ASVs were retained after chimera removal? 
- 97.54% of the trimmed ASVs were retained after chimera removal
...
- 17.78% of the total merged ASVs were retained after both trimming and chimera removal?
- At this point in time, no, I am not confident that the ASV lengths have been trimmed appropriately, as in the initial ASL length trimming step where I established a length range of 408bp - 426bp only 18.22% of the ASVs were retained! You can see in the plot below you can see how the max number of ASVs at a certain length are just above 20 000, when before the length trimming that number was above 100 000. I am confident with the chimera removal step though. I will consult Mar/Sophia moving forward on the best strategy for establishing ASV length!
...
- The distribution is roughly unimodal, centered around 20 000 reads per sample.
- There are about 7 samples with very low read counts
- A couple of other samples have higher sequencing depth (>60,000 reads).

**Panel B: ASV Richness after DADA2:** Histogram of ASV richness (Total # of ASVs per sample)

- Most samples have ~125–375 ASVs, following a similar unimodal distribution as sequencing depth.
- About 2 samples have very low ASV counts (~0-10). *Perhaps this corresponds to low read depth?*

**Panel C: Read Depth vs ASV Richness:** Scatter plot showing the relationship between sequencing depth and ASV richness.

- Similar to the class data, there generally there might be a positive correlation, where samples with higher read depth tend to have more ASVs. 

**Overall Interpretation & Considerations**

1.	*Sequencing depth variation*: Most samples cluster around a common sequencing depth, but there are at least seven samples that are definitely under-sequenced.
