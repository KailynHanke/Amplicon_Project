...
3.	*Decline in Reverse Reads After ~115 Cycles/Bases:* 
    ...
    - **Solution**: We will truncate the reads using `truncLen = 249` to remove the final poor-quality base at the end of the forward reads and `truncLen = 240` to remove the final 10 poor-quality bases at the end of the reverse reads. 
...
*Solutions Summary:*
- Forward Reads:Truncate the reads using `truncLen = 249`
- Reverse Reads:Truncate the reads using `truncLen = 240`
...
# Trimming primers: 
#   Forward (341F): 17 bp (5′-CCTACGGGNGGCWGCAG)
#   Reverse (805R): 21 bp (5′-GACTACHVGGGTATCTAATCC)

# Filtering out low quality reads (threshold = quality score ~29 - 30): 
#   Forward: Trim to base/cycle 249 to eliminate sudden drop in quality score
#   Reverse: Trim to base/cycle 240 to eliminate sudden drop in quality score
...
- **Max % Retained** is `r read_stats_df$max_percent_retained` is not great, while **min % retained is** `r read_stats_df$min_percent_retained` which isn't super great either.
- The **median % retained ** was `r read_stats_df$median_percent_retained`.
...
- *Forward Reads (Pre vs. Post-QC)*
  - *Beginning of read*: The forward primer (first 17 bases) was trimmed. This also effectively got rid of any lower quality bases that were present at the beginning of the reads. The majority of Phred scores are very good ~37. 
...
- *Reverse Reads (Pre vs. Post-QC)*
  - *Beginning of read*: The reverse primer (first 21 bases) was trimmed. This also effectively got rid of the lower quality bases that were present at the beginning of the reads. The majority of Phred scores are very good ~37 (similar to forward reads).
  - *Middle of read*: Mid-read quality remains strong until ~140 bp. Where there begins to be a gradual decline in Phred scores.
  - *End of read*: Pre-QC reads had a gradual decline in quality from ~140 cycles/bases until the end of the read length, with a steeper drop in quality at the very end of the read. Note that this gradual decline is more significant in the reverse reads vs. the forward reads. The trailing end of the reverse reads still shows some degradation, but it’s significantly improved from pre-QC by trimming those last 10 bases which were lowest quality. 
